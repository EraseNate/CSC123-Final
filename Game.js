class Game {
  constructor(epilogueInstance) {
    this.checkGameOver = this.checkGameOver.bind(this);
    this.epilogue = epilogueInstance;
    textFont(startFont);
  ...
  checkGameOver() {
    if (this.opponent.health <= 0) {
      ...
      const allDefeated = scene.opponents.every(opponent => opponent.defeated);
      if (allDefeated) {
        this.epilogue.setOutcome(true); // Use epilogue instance
        this.epilogue.epilogueActive = true; // Activate epilogue
        gameStarted = false;
      }
      ...
      this.epilogue.setOutcome(false); // Use epilogue instance
      this.epilogue.epilogueActive = true; // Activate epilogue
      gameStarted = false;
    }
  }
